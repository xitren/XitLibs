...
uint8_t buffer[HANDLER_BUFFER_LENGTH];
...
inline coap_rw_buffer_t *ext_handler(const uint8_t *buf, size_t buflen, char *ip, uint32_t port)
{
    int rc;
...
int TransferUDP(
        ...
        const uint32_t port)
{
    WiFi_SendTo((_u8 *) data, datalen);
    return 0;
}

int TransferDMA(const uint8_t *data, uint32_t datalen)
{
    if ((data[0] == 0x01) && (data[1] == 0x1C) && (data[2] == 0x20))
    {
    }
    else if ((data[0] == 0x01) && (data[1] == 0x1C) && (data[2] == 0x60))
    {
    }
    else
    {
