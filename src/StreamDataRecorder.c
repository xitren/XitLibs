...
static uint32_t signal_amplitude[BUFFER_SAMPLE_SIZE - 1] = {10, 100, 1000, 10000, 1000, 100, 10};
static uint32_t noise_amplitude[BUFFER_SAMPLE_SIZE - 1] = {100, 100, 100, 100, 100, 100, 100};
static uint32_t power_amplitude[BUFFER_SAMPLE_SIZE - 1] = {100, 1000, 10000, 1000, 1000, 1000, 1000};
		...
		uint8_t *data, uint32_t *data_size, uint32_t buffer_size)
{
	...
	if ((TempParam))
	{
		...
		{
			DBG_LOG_DEBUG("buffer_size2 %s %lu.\n", data_st, *data_size);
			...
			if ((last <= From) && (To <= first) && (From <= To))
				switch (MediaType)
				{
					case Media_JSON:
						...
						DBG_LOG_TRACE("(circularbuffer_unreaded_items_size %lu) \n", l);
						if (l > 0)
						{
							last = From;
							first = To;
		...
		} else
		{
			switch (MediaType)
			{
				...
				case Media_TEXT:
					...
					break;
				case Media_BYTE:
					DBG_LOG_DEBUG("buffer_size3 %s %lu.\n", data_st, *data_size);
					current_coap_mediatype = Media_BYTE;
					l = circularbuffer_unreaded_items_size(&buffer_test);
					if (l > 0)
					{
